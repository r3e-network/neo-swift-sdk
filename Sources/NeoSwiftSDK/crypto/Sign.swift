...
public enum Sign {
    ...
    private static func signatureData(message: Bytes, signature sig: ECDSASignature, publicKey: ECPublicKey) throws -> SignatureData {
        ...
        guard recId != -1 else { throw NeoError.runtime("Could not construct a recoverable key. This should never happen.") }
    ...
    public static func recoverFromSignature(recId: Int, sig: ECDSASignature, message: Bytes) throws -> ECPublicKey? {
        guard recId >= 0 else { throw NeoError.runtime("recId must be positive") }
        guard sig.r.signum >= 0 else { throw NeoError.runtime("r must be positive") }
        guard sig.s.signum >= 0 else { throw NeoError.runtime("s must be positive") }
        guard !message.isEmpty else { throw NeoError.runtime("message cannot be empty") }
    ...
    public static func signedMessageToKey(message: Bytes, signatureData: SignatureData) throws -> ECPublicKey {
        ...
        guard r.count == 32, s.count == 32 else {
            throw NeoError.runtime("\(r.count == 32 ? "s" : "r") must be 32 bytes.")
        }
