...
public class Wallet {
    ...
    private static let DEFAULT_WALLET_NAME = "NeoSwiftSDKWallet"
    ...
    public func defaultAccount(_ accountHash160: Hash160) throws -> Wallet {
        guard accountsMap.keys.contains(accountHash160) else {
            throw NeoError.illegalArgument("Cannot set default account on wallet. Wallet does not contain the account with script hash \(accountHash160.string).")
        }
    ...
    public func addAccounts(_ accounts: [Account]) throws -> Wallet {
        ...
        if let account = accounts.first(where: { $0.wallet != nil }) {
            throw NeoError.illegalArgument("The account \(account.address) is already contained in a wallet. Please remove this account from its containing wallet before adding it to another wallet.")
        }
    ...
    public func removeAccount(_ accountHash: Hash160) throws -> Bool {
        ...
        guard accountsMap.count > 1 else {
            throw NeoError.illegalState("The account \(accountHash.toAddress()) is the only account in the wallet. It cannot be removed.")
        }
        ...
        if accountHash == defaultAccount?.scriptHash {
            guard let newDefault = accountsMap.keys.first(where: { $0 != accountHash }) else {
                throw NeoError.illegalState("Could not find a new default account after removal")
            }
    ...
    public static func fromNEP6Wallet(_ nep6Wallet: NEP6Wallet) throws -> Wallet {
        ...
        guard let defaultAccount = defaultAccount else {
            throw NeoError.illegalArgument("The NEP-6 wallet does not contain any default account.")
        }
    ...
    public func getNep17TokenBalances(_ rpcClient: NeoRpcClient) async throws -> [Hash160: Int] {
        var balances: [Hash160: Int] = [:]
        for (_, account) in accountsMap {
            for (key, value) in try await account.getNep17Balances(rpcClient) {
    ...
    public static func withAccounts(_ accounts: [Account]) throws -> Wallet {
        guard let firstAccount = accounts.first else {
            throw NeoError.illegalState("No accounts provided to initialize a wallet.")
        }
