...
public final class TransactionBuilder {
    ...
    private(set) var attributes: [TransactionAttribute]
    ...
    public init(_ rpcClient: NeoRpcClient) {
    ...
    ///
    /// By default, it is set to the maximum, which is the current chain height plus ``NeoRpcClientConfiguration/maxValidUntilBlockIncrement``.
    ...
    public func firstSigner(_ sender: Hash160) throws -> TransactionBuilder {
        guard !signers.contains(where: { $0.scopes.contains(.none) }) else {
            throw NeoError.illegalState("This transaction contains a signer with fee-only witness scope that will cover the fees. Hence, the order of the signers does not affect the payment of the fees.")
        }
        guard let s = signers.first(where: { $0.signerHash == sender }) else {
            throw NeoError.illegalState("Could not find a signer with script hash \(sender.string). Make sure to add the signer before calling this method.")
        }
    ...
    /// - Returns: This transaction builder (self)
    public func attributes(_ attributes: [TransactionAttribute]) throws -> TransactionBuilder {
        var updatedAttributes = self.attributes
        attributes.forEach { attr in
            ...
            let alreadyPresent = updatedAttributes.contains(where: { $0.byte == attr.byte })
            if !alreadyPresent {
                updatedAttributes.append(attr)
            }
        }
        try throwIfMaxAttributesExceeded(signers.count, updatedAttributes.count)
        self.attributes = updatedAttributes
        return self
    }
    ...
    public func getUnsignedTransaction() async throws -> NeoTransaction {
        ...
        if validUntilBlock == nil {
            let currentBlockCount = try await rpcClient.getBlockCount().send().getResult()
            try _ = validUntilBlock(currentBlockCount + rpcClient.maxValidUntilBlockIncrement - 1)
        }
        guard !signers.isEmpty else {
            throw NeoError.illegalState("Cannot create a transaction without signers. At least one signer with witness scope fee-only or higher is required.")
        }
        if isHighPriority {
            let isAllowed = try await isAllowedForHighPriority()
            if !isAllowed {
                throw NeoError.illegalState("This transaction does not have a committee member as signer. Only committee members can send transactions with high priority.")
            }
        }
        ...
        return NeoTransaction(rpcClient: rpcClient, version: version, nonce: nonce,
    ...
    private func isAllowedForHighPriority() async throws -> Bool {
        let committee = try await rpcClient.getCommittee().send().getResult()
    ...
    private func getSystemFeeForScript() async throws -> Int {
        let response = try await rpcClient.invokeScript(script?.noPrefixHex ?? "", self.signers).send()
        let result = try response.getResult()
        guard !result.hasStateFault || rpcClient.config.allowsTransmissionOnFault else {
    ...
    private func calcNetworkFee() async throws -> Int {
        ...
        let tx = NeoTransaction(rpcClient: rpcClient, version: version, nonce: nonce,
                                         validUntilBlock: validUntilBlock, signers: signers,
        ...
        return try await rpcClient.calculateNetworkFee(tx.toArray().noPrefixHex).send().getResult().networkFee
    }
    ...
    private func getSenderGasBalance() async throws -> Int {
        ...
        let result = try await rpcClient.invokeFunction(Self.GAS_TOKEN_HASH, Self.BALANCE_OF_FUNCTION, [.hash160(firstSigner.signerHash)], []).send().getResult()
    ...
    public func callInvokeScript() async throws -> NeoInvokeScript {
        ...
        return try await rpcClient.invokeScript(script.noPrefixHex, signers).send()
    }
    ...
    public func doIfSenderCannotCoverFees(_ consumer: @escaping (Int, Int) -> Void) throws -> TransactionBuilder {
        guard feeError == nil else {
            throw NeoError.illegalState("Cannot handle a consumer for this case, since an exception will be thrown if the sender cannot cover the fees.")
        }
    ...
    public func throwIfSenderCannotCoverFees(_ error: Error) throws -> TransactionBuilder {
        guard consumer == nil else {
            throw NeoError.illegalState("Cannot handle a supplier for this case, since a consumer will be executed if the sender cannot cover the fees.")
        }
        feeError = error
        return self
    }
