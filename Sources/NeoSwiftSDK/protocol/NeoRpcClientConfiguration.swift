...
public class NeoRpcClientConfiguration {
    ...
    /// The interval in milliseconds in which ``NeoRpcClient/NeoRpcClient`` polls the neo-node for new block information when observing the blockchain.
    ...
    /// Sets the interval in milliseconds in which ``NeoRpcClient/NeoRpcClient`` should poll the neo-node for new block information when observing the blockchain.
    /// - Parameter pollingInterval: The polling interval in milliseconds
    /// - Returns: The config (self)
    public func setPollingInterval(_ pollingInterval: Int) -> NeoRpcClientConfiguration {
        self.pollingInterval = pollingInterval
        return self
    }
    ...
    /// - Returns: The config (self)
    public func setScheduledDispatchQueue(_ dispatchQueue: DispatchQueue) -> NeoRpcClientConfiguration {
    ...
    /// - Returns: The config (self)
    public func setNetworkMagic(_ magic: Int) throws -> NeoRpcClientConfiguration {
        guard magic <= 0xFFFFFFFF && magic >= 0 else {
            throw NeoError.illegalArgument("The network magic number must fit into a 32-bit unsigned integer, i.e., it must be positive and not greater than 0xFFFFFFFF.")
        }
    ...
    /// - Returns: The config (self)
    public func setBlockInterval(_ blockInterval: Int) -> NeoRpcClientConfiguration {
        self.blockInterval = blockInterval
        return self
    }
    ...
    /// - Returns: The config (self)
    public func setMaxValidUntilBlockIncrement(_ maxValidUntilBlockIncrement: Int) -> NeoRpcClientConfiguration {
        self.maxValidUntilBlockIncrement = maxValidUntilBlockIncrement
        return self
    }
    ...
    /// - Returns: The config (self)
    public func setNNSResolver(_ nnsResolver: Hash160) -> NeoRpcClientConfiguration {
        self.nnsResolver = nnsResolver
        return self
    }
    ...
    /// - Returns: The config (self)
    public func allowTransmissionOnFault() -> NeoRpcClientConfiguration {
    ...
    /// - Returns: The config (self)
    public func preventTransmissionOnFault() -> NeoRpcClientConfiguration {
