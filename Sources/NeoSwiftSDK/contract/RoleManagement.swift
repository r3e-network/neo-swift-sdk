...
public class RoleManagement: SmartContract {
    ...
    /// Constructs a new ``RoleManagement`` that uses the given ``NeoRpcClient/NeoRpcClient`` instance for invocations.
    /// - Parameter rpcClient: The ``NeoRpcClient/NeoRpcClient`` instance to use for invocations
    public init(_ rpcClient: NeoRpcClient) {
        super.init(scriptHash: RoleManagement.SCRIPT_HASH, rpcClient: rpcClient)
    }
    ...
    private func checkBlockIndexValidity(_ blockIndex: Int) async throws {
        guard blockIndex >= 0 else {
            throw NeoError.illegalArgument("The block index has to be positive.")
        }
        let currentBlockCount = try await rpcClient.getBlockCount().send().getResult()
        guard blockIndex <= currentBlockCount else {
            throw NeoError.illegalArgument("The provided block index (\(blockIndex)) is too high. The current block count is \(currentBlockCount).")
        }
    }
    ...
    public func designateAsRole(_ role: Role, _ pubKeys: [ECPublicKey]) throws -> TransactionBuilder {
        guard !pubKeys.isEmpty else {
            throw NeoError.illegalArgument("At least one public key is required for designation.")
        }
