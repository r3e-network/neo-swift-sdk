...
public struct NefFile {
    ...
    public init(compiler: String?, sourceUrl: String = "", methodTokens: [MethodToken], script: Bytes) throws {
        ...
        guard compilerSize <= NefFile.COMPILER_SIZE else {
            throw NeoError.illegalArgument("The compiler name and version string can be max \(NefFile.COMPILER_SIZE) bytes long, but was \(compilerSize) bytes long.")
        }
        self.compiler = compiler
        self.sourceUrl = sourceUrl
        self.methodTokens = methodTokens
        guard sourceUrl.bytes.count < NefFile.MAX_SOURCE_URL_SIZE else {
            throw NeoError.illegalArgument("The source URL must not be longer than \(NefFile.MAX_SOURCE_URL_SIZE) bytes.")
        }
        self.script = script
    ...
    public static func readFromFile(_ file: URL) throws -> NefFile {
        ...
        guard fileBytes.count <= 0x100000 else {
            throw NeoError.illegalArgument("The given NEF file is too large. File was \(fileBytes.count) bytes, but a max of 2^20 bytes is allowed.")
        }
    ...
    public struct MethodToken: NeoSerializable, Hashable {
        ...
        public static func deserialize(_ reader: BinaryReader) throws -> NefFile.MethodToken {
            do {
                ...
                throw NeoError.deserialization(error.localizedDescription)
            }
        }
...
extension NefFile: NeoSerializable {
    ...
    public static func deserialize(_ reader: BinaryReader) throws -> Self {
        ...
        guard script.count > 0 else {
            throw NeoError.deserialization("Script cannot be empty in NEF file.")
        }
        ...
        guard file.checksum == checksum else {
            throw NeoError.deserialization("NEF file checksum validation failed. Expected: \(file.checksum.toHexString()), Got: \(checksum.toHexString())")
        }
        return file
    }
