...
public class ContractManagement: SmartContract {
    ...
    public init(_ rpcClient: NeoRpcClient) {
        super.init(scriptHash: ContractManagement.SCRIPT_HASH, rpcClient: rpcClient)
    }
    ...
    ///
    /// Makes use of the RPC ``NeoRpcClient/NeoRpcClient/getContractState(_:)``
    ...
    public func getContract(_ contractHash: Hash160) async throws -> ContractState {
        return try await rpcClient.getContractState(contractHash).send().getResult()
    }
    ...
    private func getContractHashById(_ id: Int) async throws -> Hash160 {
        ...
        do {
            ...
            guard list.count >= 3 else { throw NeoError.illegalState() }
            return try Hash160(list[2].getByteArray())
        } catch {
            throw NeoError.illegalArgument("Could not get the contract hash for the provided id.")
        }
    }
    ...
    public func deploy(_ nef: NefFile, _ manifest: ContractManifest, _ data: ContractParameter? = nil) throws -> TransactionBuilder {
        ...
        guard manifestBytes.count <= NeoConstants.MAX_MANIFEST_SIZE else {
            throw NeoError.illegalArgument("The given contract manifest is too long. Manifest was \(manifestBytes.count) bytes big, but a max of \(NeoConstants.MAX_MANIFEST_SIZE) bytes is allowed.")
        }
