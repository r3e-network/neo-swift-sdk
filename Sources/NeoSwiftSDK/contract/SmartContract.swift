...
public class SmartContract {
    ...
    internal let rpcClient: NeoRpcClient
    
    /// Constructs a `SmartContract` representing the smart contract with the given script hash. Uses the given ``NeoRpcClient/NeoRpcClient`` instance for all invocations.
    /// - Parameters:
    ///   - scriptHash: The smart contract's script's hash
    ///   - rpcClient: The ``NeoRpcClient/NeoRpcClient`` instance to use for invocations
    public init(scriptHash: Hash160, rpcClient: NeoRpcClient) {
    ...
    public func invokeFunction(_ function: String, _ params: [ContractParameter?]) throws -> TransactionBuilder {
        ...
        return TransactionBuilder(rpcClient).script(script)
    }
    ...
    public func buildInvokeFunctionScript(_ function: String, _ params: [ContractParameter?]) throws -> Bytes {
        guard !function.isEmpty else {
            throw NeoError.illegalArgument("The invocation function must not be empty.")
        }
    ...
    public func callFunctionReturningIterator<T>(_ function: String, _ params: [ContractParameter] = [],
        ...
        guard let sessionId = invocationResult.sessionId else {
            throw NeoError.illegalState("No session id was found. The connected Neo node might not support sessions.")
        }
        ...
        return .init(rpcClient: rpcClient, sessionId: sessionId, iteratorId: iteratorId, mapper: mapper)
    }
    ...
    public func callFunctionAndTraverseIterator<T>(_ function: String, _ params: [ContractParameter] = [],
                                                   ...
                                                   mapper: @escaping (StackItem) throws -> T = { $0 }) async throws -> [T] {
        let iterator = try await callFunctionReturningIterator(function, params, mapper: mapper)
        let iteratorItems: [T]
        do {
            iteratorItems = try await iterator.traverse(maxIteratorResultItems)
        } catch {
    ...
    public func callFunctionAndUnwrapIterator(_ function: String, _ params: [ContractParameter], _ maxIteratorResultItems: Int, _ signers: [Signer] = []) async throws -> [StackItem] {
        ...
        let invocationResult = try await rpcClient.invokeScript(script.toHexString(), signers).send().getResult()
        try throwIfFaultState(invocationResult)
    ...
    public func callInvokeFunction(_ function: String, _ params: [ContractParameter] = [], _ signers: [Signer] = []) async throws -> NeoInvokeFunction {
        ...
        return try await rpcClient.invokeFunction(scriptHash, function, params, signers).send()
    }
    ...
    public func getManifest() async throws -> ContractManifest {
        return try await rpcClient.getContractState(scriptHash).send().getResult().manifest
    }
