...
public class Token: SmartContract {
    ...
    public static func toFractions(_ amount: Decimal, _ decimals: Int) throws -> Int {
        guard amount.scale <= decimals else {
            throw NeoError.illegalArgument("The provided amount has too many decimal points. Make sure the decimals of the provided amount do not exceed the supported token decimals.")
        }
    ...
    internal func resolveNNSTextRecord(_ name: NNSName) async throws -> Hash160 {
        let resolvedAddress = try await NeoNameService(rpcClient: rpcClient).resolve(name, .txt)
