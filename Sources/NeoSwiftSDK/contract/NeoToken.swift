...
public class NeoToken: FungibleToken {
    ...
    /// Constructs a new `NeoToken` that uses the given ``NeoRpcClient/NeoRpcClient`` instance for invocations.
    /// - Parameter rpcClient: The ``NeoRpcClient/NeoRpcClient`` instance to use for invocations
    public init(_ rpcClient: NeoRpcClient) {
        super.init(scriptHash: NeoToken.SCRIPT_HASH, rpcClient: rpcClient)
    }
    ...
    public func getAccountState(_ account: Hash160) async throws -> NeoAccountState {
        guard let result = try await callInvokeFunction(NeoToken.GET_ACCOUNT_STATE, [.hash160(account)])
            ...
            throw NeoError.illegalState("Account State stack was empty.")
        }
        ...
        guard let state = result.list, state.count >= 3,
            ...
            throw NeoError.illegalState("Account State stack was malformed.")
        }
