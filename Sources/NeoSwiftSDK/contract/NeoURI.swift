...
public class NeoURI {
    ...
    public private(set) var rpcClient: NeoRpcClient?
    ...
    public static func fromURI(_ uriString: String) throws -> NeoURI {
        ...
        guard let beginTx = baseAndQuery.first?.components(separatedBy: ":"),
            ...
            throw NeoError.illegalArgument("The provided string does not conform to the NEP-9 standard.")
        }
        ...
        if baseAndQuery.count == 2 {
            ...
            try query.forEach { singleQuery in
                ...
                guard parts.count == 2 else {
                    throw NeoError.illegalArgument("This URI contains invalid queries.")
                }
    ...
    public func buildTransferFrom(_ sender: Account) async throws -> TransactionBuilder {
        guard let rpcClient = rpcClient else {
            throw NeoError.illegalState("NeoRpcClient instance is not set.")
        }
        guard let recipient = recipient else {
            throw NeoError.illegalState("Recipient is not set.")
        }
        guard let amount = amount else {
            throw NeoError.illegalState("Amount is not set.")
        }
        guard let tokenHash = token else {
            throw NeoError.illegalState("Token is not set.")
        }
        
        let token = FungibleToken(scriptHash: tokenHash, rpcClient: rpcClient)
        let amountScale = amount.scale
        
        if isNeoToken(tokenHash) && amountScale > NeoToken.DECIMALS {
            throw NeoError.illegalArgument("The NEO token does not support any decimal places.")
        } else if isGasToken(tokenHash) && amountScale > GasToken.DECIMALS {
            throw NeoError.illegalArgument("The GAS token does not support more than \(GasToken.DECIMALS) decimal places.")
        } else {
            let decimals = try await token.getDecimals()
            if amountScale > decimals {
                throw NeoError.illegalArgument("The \(tokenHash) token does not support more than \(decimals) decimal places.")
            }
        }
    ...
    /// Sets the NeoRpcClient instance
    /// - Parameter rpcClient: The NeoRpcClient instance
    /// - Returns: This NeoURI object
    public func rpcClient(_ rpcClient: NeoRpcClient) -> NeoURI {
        self.rpcClient = rpcClient
        return self
    }
    ...
    public func buildURI() throws -> NeoURI {
        guard let recipient = recipient else {
            throw NeoError.illegalState("Could not create a NEP-9 URI without a recipient address.")
        }
