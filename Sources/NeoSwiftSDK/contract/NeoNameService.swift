...
///
/// To set a specific script hash for this contract, see ``NeoRpcClientConfiguration/setNNSResolver(_:)``.
...
public class NeoNameService: NonFungibleToken {
    ...
    ///
    /// Uses the NNS script hash specified in the ``NeoRpcClientConfiguration``.
    ...
    /// Uses the given ``NeoRpcClient`` instance for invocations.
    /// - Parameter rpcClient: The ``NeoRpcClient`` instance to use for invocations
    public init(rpcClient: NeoRpcClient) {
        super.init(scriptHash: rpcClient.nnsResolver, rpcClient: rpcClient)
    }
    ...
    public func renew(_ name: NNSName, _ years: Int) async throws -> TransactionBuilder {
        guard years > 0 && years <= 10 else {
            throw NeoError.illegalArgument("Domain names can only be renewed by at least 1, and at most 10 years.")
        }
        try await checkDomainNameAvailability(name, false)
    ...
    public func getRecord(_ name: NNSName, _ type: RecordType) async throws -> String {
        do {
            ...
            throw NeoError.illegalArgument("Could not get a record of type '\(type)' for the domain name '\(name.name)'.")
        } catch ProtocolError.invocationFaultState {
            throw NeoError.illegalArgument("The domain name '\(name.name)' might not be registered or is in an invalid format.")
        }
    }
    ...
    internal func checkDomainNameAvailability(_ name: NNSName, _ shouldBeAvailable: Bool) async throws {
        ...
        if shouldBeAvailable && !isAvailable {
            throw NeoError.illegalArgument("The domain name '\(name.name)' is already taken.")
        } else if !shouldBeAvailable && isAvailable {
            throw NeoError.illegalArgument("The domain name '\(name.name)' is not registered.")
        }
    }
    ...
    private func deserializeNameState(_ invocationResult: InvocationResult) throws -> NameState {
        ...
        guard let name = map[NeoNameService.NAME_PROPERTY]?.string else {
            throw NeoError.illegalState("'name' property not found in stack item")
        }
