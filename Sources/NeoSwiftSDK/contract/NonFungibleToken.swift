...
public class NonFungibleToken: Token {
    ...
    /// Constructs a new `NFT` representing the contract with the given script hash. Uses the given ``NeoRpcClient/NeoRpcClient`` instance for all invocations.
    /// - Parameters:
    ///   - scriptHash: The token contract's script hash
    ///   - rpcClient: The ``NeoRpcClient/NeoRpcClient`` instance to use for invocations
    public override init(scriptHash: Hash160, rpcClient: NeoRpcClient) { super.init(scriptHash: scriptHash, rpcClient: rpcClient) }
    ...
    private func throwIfDivisibleNFT() async throws {
        if try await getDecimals() != 0 {
            throw NeoError.illegalState("This method is only intended for non-divisible NFTs.")
        }
    }
    ...
    private func throwIfSenderIsNotOwner(_ from: Hash160, _ tokenId: Bytes) async throws {
        ...
        if tokenOwner != from {
            throw NeoError.illegalArgument("The provided from account is not the owner of this token.")
        }
    }
    ...
    private func throwIfNonDivisibleNFT() async throws {
        if try await getDecimals() == 0 {
            throw NeoError.illegalState("This method is only intended for divisible NFTs.")
        }
    }
