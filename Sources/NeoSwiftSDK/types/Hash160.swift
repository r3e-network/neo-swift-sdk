...
public struct Hash160: StringDecodable, Codable, Hashable, Sendable {
    ...
    public init(_ hash: Bytes) throws {
        guard hash.count == NeoConstants.HASH160_SIZE else {
            throw NeoError.illegalArgument("Hash must be \(NeoConstants.HASH160_SIZE) bytes long but was \(hash.count) bytes.")
        }
        self.hash = hash
    }
    ...
    public init(_ hash: String) throws {
        guard hash.isValidHex else {
            throw NeoError.illegalArgument("String argument is not hexadecimal.")
        }
