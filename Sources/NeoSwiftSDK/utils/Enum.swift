...
public extension ByteEnum {
    ...
    static func throwingValueOf(_ byte: Byte) throws -> Self {
        guard let value = allCases.first(where: { $0.byte == byte }) else {
            throw NeoError.illegalArgument("\(String(describing: OracleResponseCode.self)) value type not found")
        }
        return value
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        if let string = try? container.decode(String.self),
           ...
           let value = Self.valueOf(byte) {
            ...
            throw NeoError.illegalArgument("\(String(describing: Self.self)) value type not found")
        }
    }
