...
public extension Bytes {
    ...
    func toPadded(length: Int, trailing: Bool = false) throws -> Bytes {
        ...
        guard bytesLength <= length else {
            throw NeoError.illegalArgument("Input is too large to put in byte array of size \(length)")
        }
        if trailing {
    ...
    static func ^ (lhs: Bytes, rhs: Bytes) throws -> Bytes {
        guard lhs.count == rhs.count else {
            throw NeoError.illegalArgument("Arrays do not have the same length to perform the XOR operation.")
        }
