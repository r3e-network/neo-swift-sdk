...
public struct BInt: CustomStringConvertible, Comparable, Equatable, Hashable {
    ...
    static internal func randomBytes(_ bytes: inout Bytes) {
        bytes.withUnsafeMutableBytes(fillRandom)
    }
    
    static internal func randomLimbs(_ limbs: inout Limbs) {
        limbs.withUnsafeMutableBytes(fillRandom)
    }

    private static func fillRandom(_ buffer: UnsafeMutableRawBufferPointer) {
        guard let baseAddress = buffer.baseAddress, buffer.count > 0 else {
            return
        }
#if canImport(Security)
        guard SecRandomCopyBytes(kSecRandomDefault, buffer.count, baseAddress) == errSecSuccess else {
            fatalError("random source failed")
        }
#elseif os(Linux)
        ...
        while offset < buffer.count {
            let count = read(fd, baseAddress.advanced(by: offset), buffer.count - offset)
            guard count > 0 else {
                fatalError("random source failed")
            }
            offset += count
