
import Foundation
import XCTest
@testable import NeoSwiftSDK

class RequestTests: XCTestCase {
    
    // MARK: Blockchain Methods
    
    public func testGetBestBlockHash() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getbestblockhash\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBestBlockHash()
        })
    }
    
    public func testGetBlockHash() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockhash\"," +
        "\"id\":1," +
        "\"params\":[16293]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlockHash(16293)
        })
    }
    
    func testGetBlock_Index() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblock\"," +
        "\"id\":1," +
        "\"params\":[12345,1]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlock(12345, true)
        })
    }
    
    func testGetBlock_Index_onlyHeader() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockheader\"," +
        "\"id\":1," +
        "\"params\":[12345,1]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlock(12345, false)
        })
    }
    
    func testGetBlock_Hash() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblock\"," +
        "\"id\":1," +
        "\"params\":[\"2240b34669038f82ac492150d391dfc3d7fe5e3c1d34e5b547d50e99c09b468d\",1]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getBlock(Hash256("0x2240b34669038f82ac492150d391dfc3d7fe5e3c1d34e5b547d50e99c09b468d"), true)
        })
    }
    
    func testGetBlock_notFullTxObjects() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockheader\"," +
        "\"id\":1," +
        "\"params\":[\"2240b34669038f82ac492150d391dfc3d7fe5e3c1d34e5b547d50e99c09b468d\",1]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getBlock(Hash256("0x2240b34669038f82ac492150d391dfc3d7fe5e3c1d34e5b547d50e99c09b468d"), false)
        })
    }
    
    func testGetRawBlock_Index() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblock\"," +
        "\"id\":1," +
        "\"params\":[12345,0]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getRawBlock(12345)
        })
    }
    
    func testGetBlockHeaderCount() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockheadercount\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlockHeaderCount()
        })
    }
    
    public func testGetBlockCount() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockcount\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlockCount()
        })
    }
    
    public func testGetNativeContracts() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnativecontracts\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getNativeContracts()
        })
    }
    
    public func testGetBlockHeader_Index() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockheader\"," +
        "\"id\":1," +
        "\"params\":[12345,1]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getBlockHeader(12345)
        })
    }
    
    public func testGetRawBlockHeader_Index() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getblockheader\"," +
        "\"id\":1," +
        "\"params\":[12345,0]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getRawBlockHeader(12345)
        })
    }
    
    public func testGetContractState() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getcontractstate\"," +
        "\"id\":1," +
        "\"params\":[\"dc675afc61a7c0f7b3d2682bf6e1d8ed865a0e5f\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getContractState(Hash160("dc675afc61a7c0f7b3d2682bf6e1d8ed865a0e5f"))
        })
    }
    
    public func testGetContractState_byName() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getcontractstate\"," +
        "\"id\":1," +
        "\"params\":[\"NeoToken\"]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getNativeContractState("NeoToken")
        })
    }
    
    public func testGetMemPool() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawmempool\"," +
        "\"id\":1," +
        "\"params\":[1]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getMemPool()
        })
    }
    
    public func testGetRawMemPool() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawmempool\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getRawMemPool()
        })
    }
    
    public func testGetTransaction() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawtransaction\"," +
        "\"id\":1," +
        "\"params\":[\"1f31821787b0a53df0ff7d6e0e7ecba3ac19dd517d6d2ea5aaf00432c20831d6\",1]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getTransaction(Hash256("0x1f31821787b0a53df0ff7d6e0e7ecba3ac19dd517d6d2ea5aaf00432c20831d6"))
        })
    }
    
    public func testGetRawTransaction() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawtransaction\"," +
        "\"id\":1," +
        "\"params\":[\"1f31821787b0a53df0ff7d6e0e7ecba3ac19dd517d6d2ea5aaf00432c20831d6\",0]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getRawTransaction(Hash256("0x1f31821787b0a53df0ff7d6e0e7ecba3ac19dd517d6d2ea5aaf00432c20831d6"))
        })
    }
    
    public func testGetStorage() {
        let key = "616e797468696e67"
        let hash = try! Hash160("03febccf81ac85e3d795bc5cbd4e84e907812aa3")
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getstorage\"," +
        "\"id\":1," +
        "\"params\":[\"03febccf81ac85e3d795bc5cbd4e84e907812aa3\",\"" + key.base64Encoded + "\"]}"
        verifyRequest(json) { rpcClient in
            return rpcClient.getStorage(hash, key)
        }
    }
    
    public func testGetTransactionHeight() {
        let hash = try! Hash256("0x793f560ae7058a50c672890e69c9292391dd159ce963a33462059d03b9573d6a")
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"gettransactionheight\"," +
        "\"id\":1," +
        "\"params\":[\"793f560ae7058a50c672890e69c9292391dd159ce963a33462059d03b9573d6a\"]}"
        verifyRequest(json) { rpcClient in
            return rpcClient.getTransactionHeight(hash)
        }
    }
    
    public func testGetNextBlockValidators() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnextblockvalidators\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getNextBlockValidators()
        })
    }
    
    public func testGetCommittee() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getcommittee\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getCommittee()
        })
    }
    
    // MARK: Node Methods
    
    public func testGetConnectionCount() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getconnectioncount\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getConnectionCount()
        })
    }
    
    public func testGetPeers() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getpeers\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getPeers()
        })
    }
    
    public func testGetVersion() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getversion\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getVersion()
        })
    }

    public func testGetNetworkMagicNumberSyncsAddressVersionToClientConfigOnly() async throws {
        let originalGlobalAddressVersion = NeoRpcClientConfiguration.addressVersion
        defer { NeoRpcClientConfiguration.setAddressVersion(originalGlobalAddressVersion) }
        let json = """
{
    "jsonrpc": "2.0",
    "id": 1,
    "result": {
        "tcpport": 10333,
        "nonce": 1234,
        "useragent": "/Neo:3.10.0/",
        "protocol": {
            "network": 860833102,
            "validatorscount": 7,
            "msperblock": 15000,
            "maxvaliduntilblockincrement": 5760,
            "maxtraceableblocks": 2102400,
            "addressversion": 23,
            "maxtransactionsperblock": 512,
            "memorypoolmaxtransactions": 50000,
            "initialgasdistribution": 5200000000000000
        }
    }
}
"""
        let mockUrlSession = MockURLSession().data(["getversion": Data(json.utf8)])
        let config = NeoRpcClientConfiguration()
        let rpcClient = NeoRpcClient.build(HttpService(urlSession: mockUrlSession), config)

        let networkMagic = try await rpcClient.getNetworkMagicNumber()

        XCTAssertEqual(networkMagic, 860833102)
        XCTAssertEqual(config.addressVersion, 23)
        XCTAssertEqual(NeoRpcClientConfiguration.addressVersion, originalGlobalAddressVersion)
    }
    
    public func testSendRawTransaction() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendrawtransaction\"," +
        "\"id\":1," +
        "\"params\":[\"\"]}"
        
        verifyRequest(json) { rpcClient in
            return rpcClient
                .sendRawTransaction("")
                
        }
    }
    
    public func testRelay() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"relay\"," +
        "\"id\":1," +
        "\"params\":[{\"network\":894710606,\"type\":\"Neo.Network.P2P.Payloads.Transaction\",\"hash\":\"0xabc\",\"data\":\"AQ==\",\"items\":{\"0x01\":{\"script\":\"EQ==\",\"signatures\":{\"02ec\":\"c2ln\"},\"parameters\":[{\"type\":\"Signature\"}]}}}]}"
        
        let context = ContractParametersContext(
            hash: "0xabc",
            data: "AQ==",
            items: [
                "0x01": .init(
                    script: "EQ==",
                    parameters: [ContractParameter(type: .signature)],
                    signatures: ["02ec": "c2ln"]
                )
            ],
            network: 894710606
        )
        verifyRequest(json) { rpcClient in
            return rpcClient.relay(context)
        }
    }
    
    public func testSubmitBlock() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"submitblock\"," +
        "\"id\":1," +
        "\"params\":[\"00000000000000000000000000000000\"]}"
        
        
        verifyRequest(json, { rpcClient in
            return rpcClient.submitBlock("00000000000000000000000000000000")
        })
    }
    
    // MARK: Smart Contract Methods
    
    public func testInvokeFunction() {
        let pubKey = try! ECPublicKey(defaultAccountPublicKey)
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokefunction\"," +
        "\"id\":1," +
        "\"params\":[\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\",\"balanceOf\"," +
        "[{\"type\":\"Hash160\",\"value\":\"91b83e96f2a7c4fdf0c1688441ec61986c7cae26\"}]," +
        "[{\"allowedcontracts\":[\"ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5\"]," +
        "\"account\":\"cadb3dc2faa3ef14a13b619c9a43124755aa2569\"," +
        "\"rules\":[{" +
        "\"condition\":{" +
        "\"type\":\"CalledByContract\"," +
        "\"hash\":\"" + neoTokenHash + "\"}," +
        "\"action\":\"Allow\"}]," +
        "\"allowedgroups" +
        "\":[\"033a4d051b04b7fc0230d2b1aaedfd5a84be279a5361a7358db665ad7857787f1b\"]," +
        "\"scopes\":\"CalledByEntry,CustomContracts,CustomGroups,WitnessRules\"" +
        "}]]}"
        
        verifyRequest(json) { rpcClient in
            return try! rpcClient.invokeFunction(
                Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"),
                "balanceOf",
                [.hash160(Hash160("91b83e96f2a7c4fdf0c1688441ec61986c7cae26"))],
                [AccountSigner.calledByEntry(Hash160("0xcadb3dc2faa3ef14a13b619c9a43124755aa2569"))
                    .setAllowedContracts([Hash160(neoTokenHash)])
                    .setAllowedGroups([pubKey])
                    .setRules([.init(action: .allow, condition: .calledByContract(Hash160(neoTokenHash)))])
                ]
            )
        }
    }
    
    public func testInvokeFunction_witnessRules() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokefunction\"," +
        "\"id\":1," +
        "\"params\":[\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\",\"balanceOf\"," +
        "[{\"type\":\"Hash160\",\"value\":\"91b83e96f2a7c4fdf0c1688441ec61986c7cae26\"}]," +
        "[{\"allowedcontracts\":[\"ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5\"]," +
        "\"account\":\"cadb3dc2faa3ef14a13b619c9a43124755aa2569\"," +
        "\"rules\":[{" +
        "\"condition\":{" +
        "\"type\":\"And\"," +
        "\"expressions\":[{" +
        "\"type\":\"Boolean\"," +
        "\"expression\":true" +
        "},{" +
        "\"type\":\"CalledByContract\"," +
        "\"hash\":\"" + neoTokenHash + "\"" +
        "},{" +
        "\"type\":\"CalledByGroup\"," +
        "\"group\":\"" + defaultAccountPublicKey + "\"" +
        "},{" +
        "\"type\":\"Group\"," +
        "\"group\":\"" + defaultAccountPublicKey + "\"" +
        "}]}," +
        "\"action\":\"Deny\"" +
        "},{" +
        "\"condition\":{" +
        "\"type\":\"Or\"," +
        "\"expressions\":[" +
        "{\"type\":\"CalledByGroup\"," +
        "\"group\":\"" + defaultAccountPublicKey + "\"" +
        "},{" +
        "\"type\":\"ScriptHash\"," +
        "\"hash\":\"" + committeeAccountScriptHash + "\"" +
        "}]}," +
        "\"action\":\"Deny\"" +
        "},{" +
        "\"condition\":{" +
        "\"type\":\"Not\"," +
        "\"expression\":{" +
        "\"type\":\"CalledByEntry\"" +
        "}}," +
        "\"action\":\"Allow\"" +
        "}]," +
        "\"allowedgroups" +
        "\":[\"033a4d051b04b7fc0230d2b1aaedfd5a84be279a5361a7358db665ad7857787f1b\"]," +
        "\"scopes\":\"CalledByEntry,CustomContracts,CustomGroups,WitnessRules\"" +
        "}]]}"
        
        let pubKey = try! ECPublicKey(defaultAccountPublicKey)
        
        verifyRequest(json) { rpcClient in
            return try! rpcClient.invokeFunction(
                Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"),
                "balanceOf",
                [.hash160(Hash160("91b83e96f2a7c4fdf0c1688441ec61986c7cae26"))],
                [AccountSigner.calledByEntry(Hash160("0xcadb3dc2faa3ef14a13b619c9a43124755aa2569"))
                    .setAllowedContracts([Hash160(neoTokenHash)])
                    .setAllowedGroups([pubKey])
                    .setRules([
                        .init(action: .deny, condition: .and([.boolean(true), .calledByContract(Hash160(neoTokenHash)), .calledByGroup(pubKey), .group(pubKey)])),
                        .init(action: .deny, condition: .or([.calledByGroup(pubKey), .scriptHash(Hash160(committeeAccountScriptHash))])),
                        .init(action: .allow, condition: .not(.calledByEntry))
                    ])
                ]
            )
        }
    }
    
    public func testInvokeFunctionDiagnostics() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokefunction\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\"," +
        "\"balanceOf\"," +
        "[{" +
        "\"type\":\"Hash160\"," +
        "\"value\":\"91b83e96f2a7c4fdf0c1688441ec61986c7cae26\"" +
        "}]," +
        "[]," +
        "true" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeFunctionDiagnostics(
                Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"), "balanceOf",
                [.hash160(Hash160("91b83e96f2a7c4fdf0c1688441ec61986c7cae26"))])
        })
    }
    
    public func testInvokeFunctionDiagnostics_noParams() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokefunction\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\"," +
        "\"symbol\"," +
        "[]," +
        "[]," +
        "true" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeFunctionDiagnostics(
                Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"), "symbol", [], [])
        })
        
    }
    
    public func testInvokeScript() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokescript\"," +
        "\"id\":1," +
        "\"params\":[\"EMAMCGRlY2ltYWxzDBQlBZ7LSHjTqHX5HFHO3tMw1Fdf3kFifVtS\",[]]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.invokeScript("10c00c08646563696d616c730c1425059ecb4878d3a875f91c51ceded330d4575fde41627d5b52")
        })
    }
    
    public func testInvokeScriptDiagnostics() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokescript\"," +
        "\"id\":1," +
        "\"params\":[\"EMAMCGRlY2ltYWxzDBQlBZ7LSHjTqHX5HFHO3tMw1Fdf3kFifVtS\",[],true]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.invokeScriptDiagnostics("10c00c08646563696d616c730c1425059ecb4878d3a875f91c51ceded330d4575fde41627d5b52")
        })
    }
    
    public func testInvokeScriptWithSigner() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokescript\",\"id\":1," +
        "\"params\":[\"EMAMCGRlY2ltYWxzDBQlBZ7LSHjTqHX5HFHO3tMw1Fdf3kFifVtS\"," +
        "[{\"allowedcontracts\":[],\"account\":\"cc45cc8987b0e35371f5685431e3c8eeea306722\"," +
        "\"rules\":[],\"allowedgroups\":[],\"scopes\":\"CalledByEntry\"}]]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeScript("10c00c08646563696d616c730c1425059ecb4878d3a875f91c51ceded330d4575fde41627d5b52",
                                                 [AccountSigner.calledByEntry(Hash160("0xcc45cc8987b0e35371f5685431e3c8eeea306722"))])
        })
    }
    
    public func testInvokeScriptDiagnosticsWithSigner() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokescript\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"EMAMCGRlY2ltYWxzDBQlBZ7LSHjTqHX5HFHO3tMw1Fdf3kFifVtS\"," +
        "[{" +
        "\"allowedcontracts\":[]," +
        "\"account\":\"cc45cc8987b0e35371f5685431e3c8eeea306722\"," +
        "\"rules\":[]," +
        "\"allowedgroups\":[]," +
        "\"scopes\":\"CalledByEntry\"" +
        "}],true]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeScriptDiagnostics("10c00c08646563696d616c730c1425059ecb4878d3a875f91c51ceded330d4575fde41627d5b52",
                                                            [AccountSigner.calledByEntry(Hash160("0xcc45cc8987b0e35371f5685431e3c8eeea306722"))])
        })
        
    }
    
    public func testTraverseIterator() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"traverseiterator\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"127d3320-db35-48d5-b6d3-ca22dca4a370\"," +
        "\"cb7ef774-1ade-4a83-914b-94373ca92010\"," +
        "100" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.traverseIterator(
                "127d3320-db35-48d5-b6d3-ca22dca4a370",
                "cb7ef774-1ade-4a83-914b-94373ca92010",
                100
            )
        })
    }
    
    public func testTerminateSession() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"terminatesession\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"127d3320-db35-48d5-b6d3-ca22dca4a370\"" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.terminateSession("127d3320-db35-48d5-b6d3-ca22dca4a370")
        })
    }
    
    public func testInvokeContractVerify() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokecontractverify\"," +
        "\"id\":1," +
        "\"params\":[\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\"," +
        "[{\"type\":\"String\",\"value\":\"a string\"}," +
        "{\"type\":\"String\",\"value\":\"another string\"}]," +
        "[{\"allowedcontracts\":[],\"account\":\"cadb3dc2faa3ef14a13b619c9a43124755aa2569\"," +
        "\"rules\":[],\"allowedgroups\":[],\"scopes\":\"CalledByEntry\"" +
        "}]]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeContractVerify(
                Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"),
                [.string("a string"), .string("another string")],
                [AccountSigner.calledByEntry(Hash160("cadb3dc2faa3ef14a13b619c9a43124755aa2569"))])
        })
    }
    
    public func testInvokeContractVerifyNoParamsNoSigners() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"invokecontractverify\"," +
        "\"id\":1," +
        "\"params\":[\"af7c7328eee5a275a3bcaee2bf0cf662b5e739be\",[],[]]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.invokeContractVerify(Hash160("af7c7328eee5a275a3bcaee2bf0cf662b5e739be"))
        })
    }
    
    // MARK: Utilities Methods
    
    public func testListPlugins() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"listplugins\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        
        verifyRequest(json, { rpcClient in
            return rpcClient.listPlugins()
        })
    }
    
    public func testValidateAddress() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"validateaddress\"," +
        "\"id\":1," +
        "\"params\":[\"NTzVAPBpnUUCvrA6tFPxBHGge8Kyw8igxX\"]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.validateAddress("NTzVAPBpnUUCvrA6tFPxBHGge8Kyw8igxX")
        })
    }
    
    // MARK: Wallet Methods
    
    public func testCloseWallet() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"closewallet\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.closeWallet()
        })
    }
    
    public func testOpenWallet() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"openwallet\"," +
        "\"id\":1," +
        "\"params\":[\"wallet.json\",\"one\"]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.openWallet("wallet.json", "one")
        })
    }
    
    public func testDumpPrivKey() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"dumpprivkey\"," +
        "\"id\":1," +
        "\"params\":[\"NdWaiUoBWbPxGsm5wXPjXYJxCyuY1Zw8uW\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.dumpPrivKey(Hash160("c11d816956b6682c3406bb99b7ec8a3e93f005c1"))
        })
    }
    
    public func testSignMessage() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"signmsg\"," +
        "\"id\":1," +
        "\"params\":[\"hello\",true]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.signMessage("hello", true)
        })
    }
    
    public func testVerifyMessage() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"verifymsg\"," +
        "\"id\":1," +
        "\"params\":[\"hello\",\"signature\",\"publicKey\",\"salt\",false]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.verifyMessage("hello", "signature", "publicKey", "salt", false)
        })
    }
    
    public func testSignContext() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sign\"," +
        "\"id\":1," +
        "\"params\":[{\"network\":894710606,\"type\":\"Neo.Network.P2P.Payloads.Transaction\",\"hash\":\"0xabc\",\"data\":\"AQ==\",\"items\":{}}]}"
        
        let context = ContractParametersContext(hash: "0xabc", data: "AQ==", items: nil, network: 894710606)
        verifyRequest(json, { rpcClient in
            return rpcClient.sign(context)
        })
    }
    
    public func testGetWalletBalance() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getwalletbalance\"," +
        "\"id\":1," +
        "\"params\":[\"de5f57d430d3dece511cf975a8d37848cb9e0525\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getWalletBalance(Hash160("de5f57d430d3dece511cf975a8d37848cb9e0525"))
        })
    }
    
    public func testGetNewAddress() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnewaddress\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getNewAddress()
        })
    }
    
    public func testGetWalletUnclaimedGas() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getwalletunclaimedgas\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getWalletUnclaimedGas()
        })
    }
    
    public func testGetUnclaimedGas() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getunclaimedgas\"," +
        "\"id\":1," +
        "\"params\":[\"NaQ6Kj6qYinh1frv1wrn53wbPFe5BH5T7g\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getUnclaimedGas(Hash160("ffa6adbb5f82ad2a1aafa22ce6aaf05dad5de39e"))
        })
    }
    
    public func testImportPrivKey() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"importprivkey\"," +
        "\"id\":1," +
        "\"params\":[\"L5c6jz6Rh8arFJW3A5vg7Suaggo28ApXVF2EPzkAXbm94ThqaA6r\"]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.importPrivKey("L5c6jz6Rh8arFJW3A5vg7Suaggo28ApXVF2EPzkAXbm94ThqaA6r")
        })
    }
    
    public func testCalculateNetworkFee() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"calculatenetworkfee\"," +
        "\"id\":1," +
        "\"params\":[\"bmVvdzNq\"]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.calculateNetworkFee("6e656f77336a")
        })
    }
    
    public func testListAddress() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"listaddress\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.listAddress()
        })
    }
    
    public func testSendFrom() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendfrom\"," +
        "\"id\":1," +
        "\"params\":[\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"NaxePjypvtsQ5GVi6S1jBsSjXribTSUKRu\"," +
        "\"NbD6be5uYezFZRSBDt6aBfYR9bYsAk8Yui\"," +
        "10]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendFrom(
                Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                Hash160("8cdb257b8873049918fe5a1e7f6289f75d720ba5"),
                Hash160("db1acbae4dbae55f8325724cf080ed782925c7a7"), 10)
        })
        
    }
    
    public func testSendFrom_TransactionSendAsset() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendfrom\"," +
        "\"id\":1," +
        "\"params\":[\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"Ng9E3D4DpM6JrgSxizhanJ6zm6BjvZ2XkM\"," +
        "\"NUokBS9rfH8qncwFdfByBTT9yJjxQv8h2h\"," +
        "10" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendFrom(Hash160("44b159ceed1bfbd753748227309428f54f52e4dd"),
                                             TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                                                  value: 10, address: "NUokBS9rfH8qncwFdfByBTT9yJjxQv8h2h"))
        })
    }
    
    public func testSendMany() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendmany\"," +
        "\"id\":1," +
        "\"params\":[" +
        "[{\"asset\":\"de5f57d430d3dece511cf975a8d37848cb9e0525\",\"value\":100,\"address\":\"NRkkHsxkzFxGz77mJtJgYZ3FnBm8baU5Um\"}," +
        "{\"asset\":\"de5f57d430d3dece511cf975a8d37848cb9e0525\",\"value\":10,\"address\":\"NNFGNNK1HXSSnA7yKLzRpr8YXwcdgTrsCu\"}]" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendMany([
                TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                     value: 100, address: "NRkkHsxkzFxGz77mJtJgYZ3FnBm8baU5Um"),
                TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                     value: 10, address: "NNFGNNK1HXSSnA7yKLzRpr8YXwcdgTrsCu")
            ])
        })
        
    }
    
    public func testSendMany_Empty_Transaction() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendmany\"," +
        "\"id\":1," +
        "\"params\":[[]]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.sendMany([])
        })
    }
    
    public func testSendManyWithFrom() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendmany\"," +
        "\"id\":1," +
        "\"params\":[\"NiVNRW6cBXwkvrZnetZToaHPGSSGgV1HmA\"," +
        "[{" +
        "\"asset\":\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"value\":100," +
        "\"address\":\"Nhsi2q3hkByxcH2uBQw7cjc2qEpzXSEKTC\"" +
        "},{" +
        "\"asset\":\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"value\":10," +
        "\"address\":\"NcwVWxJZh9fxncJ9Sq8msVLotJDsAD3ZD8\"}" +
        "]]" +
        "}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendMany(
                .fromAddress("NiVNRW6cBXwkvrZnetZToaHPGSSGgV1HmA"),
                [TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                      value: 100, address: "Nhsi2q3hkByxcH2uBQw7cjc2qEpzXSEKTC"),
                 TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                      value: 10, address: "NcwVWxJZh9fxncJ9Sq8msVLotJDsAD3ZD8")]
            )
        })
    }
    
    public func testSendToAddress() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendtoaddress\"," +
        "\"id\":1," +
        "\"params\":[\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"NRCcuUUxKCa3sp45o7bjXetyxUeq58T4ED\"," +
        "10" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendToAddress(Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                                  Hash160("674231bd321880fc5c4a73994c87870e52c5fe39"), 10)
        })
    }
    
    public func testSendToAddress_TransactionSendAsset() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendtoaddress\"," +
        "\"id\":1," +
        "\"params\":[\"de5f57d430d3dece511cf975a8d37848cb9e0525\"," +
        "\"NaCsFrmoJepqCJSxnTyb41CXVSjr3dMjuL\"," +
        "10" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.sendToAddress(TransactionSendToken(token: Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525"),
                                                                       value: 10, address: "NaCsFrmoJepqCJSxnTyb41CXVSjr3dMjuL"))
        })
    }
    
    // MARK: Nep17
    
    public func testGetNep17Transfers() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep17transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NekZLTu93WgrdFHxzBEJUYgLTQMAT85GLi\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep17Transfers(Hash160("04457ce4219e462146ac00b09793f81bc5bca2ce"))
        })
    }
    
    public func testGetNep17Transfers_Date() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep17transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NekZLTu93WgrdFHxzBEJUYgLTQMAT85GLi\",1553105830]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep17Transfers(Hash160("04457ce4219e462146ac00b09793f81bc5bca2ce"),
                                                      Date(timeIntervalSince1970: 1553105830 / 1000))
        })
    }
    
    public func testGetNep17Transfers_DateFromTo() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep17transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NekZLTu93WgrdFHxzBEJUYgLTQMAT85GLi\",1553105830,1557305830]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep17Transfers(Hash160("04457ce4219e462146ac00b09793f81bc5bca2ce"),
                                                      Date(timeIntervalSince1970: 1553105830 / 1000),
                                                      Date(timeIntervalSince1970: 1557305830 / 1000))
        })
    }
    
    public func testGetNep17Balances() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep17balances\"," +
        "\"id\":1," +
        "\"params\":[\"NY9zhKwcmht5cQJ3oRqjJGo3QuVLwXwTzL\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep17Balances(Hash160("5d75775015b024970bfeacf7c6ab1b0ade974886"))
        })
    }

    public func testClientAddressVersionIsUsedForAddressRpcParameters() {
        let addressVersion: Byte = 0x17
        let scriptHash = try! Hash160("5d75775015b024970bfeacf7c6ab1b0ade974886")
        let expectedAddress = scriptHash.toAddress(addressVersion: addressVersion)
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep17balances\"," +
        "\"id\":1," +
        "\"params\":[\"\(expectedAddress)\"]}"

        verifyRequest(json, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.getNep17Balances(scriptHash)
        }
    }

    public func testClientAddressVersionIsUsedForWalletAddressRpcParameters() {
        let addressVersion: Byte = 0x17
        let scriptHash = try! Hash160("c11d816956b6682c3406bb99b7ec8a3e93f005c1")
        let expectedAddress = scriptHash.toAddress(addressVersion: addressVersion)

        let dumpPrivKeyJson = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"dumpprivkey\"," +
        "\"id\":1," +
        "\"params\":[\"\(expectedAddress)\"]}"
        verifyRequest(dumpPrivKeyJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.dumpPrivKey(scriptHash)
        }

        let unclaimedGasJson = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getunclaimedgas\"," +
        "\"id\":1," +
        "\"params\":[\"\(expectedAddress)\"]}"
        verifyRequest(unclaimedGasJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.getUnclaimedGas(scriptHash)
        }
    }

    public func testClientAddressVersionIsUsedForWalletTransferRpcParameters() {
        let addressVersion: Byte = 0x17
        let tokenHash = try! Hash160("0xde5f57d430d3dece511cf975a8d37848cb9e0525")
        let fromHash = try! Hash160("8cdb257b8873049918fe5a1e7f6289f75d720ba5")
        let toHash = try! Hash160("db1acbae4dbae55f8325724cf080ed782925c7a7")
        let fromAddress = fromHash.toAddress(addressVersion: addressVersion)
        let toAddress = toHash.toAddress(addressVersion: addressVersion)

        let sendFromJson = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendfrom\"," +
        "\"id\":1," +
        "\"params\":[\"\(tokenHash.string)\",\"\(fromAddress)\",\"\(toAddress)\",10]}"
        verifyRequest(sendFromJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.sendFrom(tokenHash, fromHash, toHash, 10)
        }

        verifyRequest(sendFromJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            try rpcClient.sendFrom(fromHash, TransactionSendToken(token: tokenHash, value: 10, address: toAddress))
        }

        let sendManyJson = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendmany\"," +
        "\"id\":1," +
        "\"params\":[\"\(fromAddress)\",[{\"asset\":\"\(tokenHash.string)\",\"value\":10,\"address\":\"\(toAddress)\"}]]}"
        verifyRequest(sendManyJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.sendMany(fromHash, [TransactionSendToken(token: tokenHash, value: 10, address: toAddress)])
        }

        let sendToAddressJson = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"sendtoaddress\"," +
        "\"id\":1," +
        "\"params\":[\"\(tokenHash.string)\",\"\(toAddress)\",10]}"
        verifyRequest(sendToAddressJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            rpcClient.sendToAddress(tokenHash, toHash, 10)
        }

        verifyRequest(sendToAddressJson, config: .init(addressVersion: addressVersion)) { rpcClient in
            try rpcClient.sendToAddress(TransactionSendToken(token: tokenHash, value: 10, address: toAddress))
        }
    }
    
    // MARK: ApplicationLogs
    
    public func testGetApplicationLog() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getapplicationlog\"," +
        "\"id\":1," +
        "\"params\":[\"420d1eb458c707d698c6d2ba0f91327918ddb3b7bae2944df070f3f4e579078b\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getApplicationLog(Hash256("420d1eb458c707d698c6d2ba0f91327918ddb3b7bae2944df070f3f4e579078b"))
        })
    }
    
    // MARK: StateService
    
    public func testGetStateRoot() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getstateroot\"," +
        "\"id\":1," +
        "\"params\":[52]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getStateRoot(52)
        })
    }
    
    public func testGetProof() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getproof\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca\"," +
        "\"79bcd398505eb779df6e67e4be6c14cded08e2f2\"," +
        "\"YW55dGhpbmc=\"" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getProof(
                Hash256("0x7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca"),
                Hash160("0x79bcd398505eb779df6e67e4be6c14cded08e2f2"),
                "616e797468696e67"
            )
        })
    }
    
    public func testVerifyProof() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"verifyproof\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca\"," +
        "\"\"" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.verifyProof(Hash256("0x7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca"),
                                                ""
            )
        })
    }
    
    public func testGetStateHeight() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getstateheight\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getStateHeight()
        })
    }
    
    public func testGetState() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getstate\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca\"," +
        "\"7c5832ba81fd0af40ec11e96b1c26613466dae02\"," +
        "\"QQEhB4DRxWFfeRI=\"" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getState(Hash256("0x7bf925dbd33af0e00d392b92313da59369ed86c82494d0e02040b24faac0a3ca"),
                                             Hash160("7c5832ba81fd0af40ec11e96b1c26613466dae02"), "4101210780d1c5615f7912")
        })
    }
    
    public func testFindStates() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"findstates\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f\"," +
        "\"d2a4cff31913016155e38e474a2c06d08be276cf\"," +
        "\"C/4=\"," +
        "\"Cw==\"," +
        "2" +
        "]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.findStates(Hash256("0x76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f"),
                                               Hash160("0xd2a4cff31913016155e38e474a2c06d08be276cf"),
                                               "0bfe",
                                               "0b",
                                               2)
        })
    }
    
    public func testFindStates_noCount() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"findstates\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f\"," +
        "\"d2a4cff31913016155e38e474a2c06d08be276cf\"," +
        "\"C/4=\"," +
        "\"Cw==\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.findStates(Hash256("0x76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f"),
                                               Hash160("0xd2a4cff31913016155e38e474a2c06d08be276cf"),
                                               "0bfe", "0b")
        })
    }
    
    public func testFindStates_noStartKey_withCount() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"findstates\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f\"," +
        "\"d2a4cff31913016155e38e474a2c06d08be276cf\"," +
        "\"C/4=\"," +
        "\"\"," +
        "53]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.findStates(Hash256("0x76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f"),
                                               Hash160("0xd2a4cff31913016155e38e474a2c06d08be276cf"),
                                               "0bfe", 53)
        })
    }
    
    public func testFindStates_noStartKey() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"findstates\"," +
        "\"id\":1," +
        "\"params\":[" +
        "\"76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f\"," +
        "\"d2a4cff31913016155e38e474a2c06d08be276cf\"," +
        "\"C/4=\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.findStates(Hash256("0x76d6bddf6d9b5979d532877f0617bf31abd03d663c73357dfb2e2417a287b09f"),
                                               Hash160("0xd2a4cff31913016155e38e474a2c06d08be276cf"),
                                               "0bfe")
        })
    }
    
    // MARK: DeferredRelay
    
    public func testGetPendingValidUntilRelay() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getpendingvaliduntilrelay\"," +
        "\"id\":1," +
        "\"params\":[]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getPendingValidUntilRelay()
        })
    }
    
    public func testGetRawPendingTransaction() {
        let txHash = try! Hash256("0x9f0c9be9507d545b4ed3311d3c5efed7c3df91d3502149ac828f036946b34022")
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawpendingtx\"," +
        "\"id\":1," +
        "\"params\":[\"9f0c9be9507d545b4ed3311d3c5efed7c3df91d3502149ac828f036946b34022\",false]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getRawPendingTransaction(txHash)
        })
    }
    
    public func testGetPendingTransaction() {
        let txHash = try! Hash256("0x9f0c9be9507d545b4ed3311d3c5efed7c3df91d3502149ac828f036946b34022")
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getrawpendingtx\"," +
        "\"id\":1," +
        "\"params\":[\"9f0c9be9507d545b4ed3311d3c5efed7c3df91d3502149ac828f036946b34022\",true]}"
        
        verifyRequest(json, { rpcClient in
            return rpcClient.getPendingTransaction(txHash)
        })
    }
    
    // MARK: Neo-express related tests
    
    public func testExpressGetPopulatedBlocks() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expressgetpopulatedblocks\"," +
        "\"id\":1," +
        "\"params\":[]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressGetPopulatedBlocks()
        })
    }
    
    public func testExpressGetNep17Contracts() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expressgetnep17contracts\"," +
        "\"id\":1," +
        "\"params\":[]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressGetNep17Contracts()
        })
    }
    
    public func testExpressGetContractStorage() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expressgetcontractstorage\"," +
        "\"id\":1," +
        "\"params\":[\"d2a4cff31913016155e38e474a2c06d08be276cf\"]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return try! rpcExpressClient.expressGetContractStorage(Hash160("0xd2a4cff31913016155e38e474a2c06d08be276cf"))
        })
    }
    
    public func testExpressListContracts() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expresslistcontracts\"," +
        "\"id\":1," +
        "\"params\":[]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressListContracts()
        })
    }
    
    
    public func testExpressCreateCheckpoint() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expresscreatecheckpoint\"," +
        "\"id\":1," +
        "\"params\":[\"checkpoint-1.neoxp-checkpoint\"]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressCreateCheckpoint("checkpoint-1.neoxp-checkpoint")
        })
    }
    
    public func testExpressListOracleRequests() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expresslistoraclerequests\"," +
        "\"id\":1," +
        "\"params\":[]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressListOracleRequests()
        })
    }
    
    public func testExpressCreateOracleResponseTx() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expresscreateoracleresponsetx\"," +
        "\"id\":1," +
        "\"params\":[{\"id\":3,\"result\":\"bmVvdzNq\",\"code\":\"Success\",\"type\":\"OracleResponse\"}]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressCreateOracleResponseTx(.oracleResponse(3, .success, "bmVvdzNq"))
        })
    }
    
    public func testExpressShutdown() {
        let json = "{" +
        "\"jsonrpc\":\"2.0\"," +
        "\"method\":\"expressshutdown\"," +
        "\"id\":1," +
        "\"params\":[]}"

        verifyExpressRequest(json, { rpcExpressClient in
            return rpcExpressClient.expressShutdown()
        })
    }
    
    // MARK: Nep11
    
    public func testGetNep11Balances() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep11balances\"," +
        "\"id\":1," +
        "\"params\":[\"NY9zhKwcmht5cQJ3oRqjJGo3QuVLwXwTzL\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep11Balances(Hash160("5d75775015b024970bfeacf7c6ab1b0ade974886"))
        })
    }
    
    public func testGetNep11Transfers() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep11transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NekZLTu93WgrdFHxzBEJUYgLTQMAT85GLi\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep11Transfers(Hash160("04457ce4219e462146ac00b09793f81bc5bca2ce"))
        })
    }
    
    public func testGetNep11Transfers_Date() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep11transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NSH1UeM96PKhjuzVBKcyWeNNuQkT3sHGmA\",1553105830]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep11Transfers(Hash160("8bed27d0e88266807a6339270f0593510967cb45"),
                                                      Date(timeIntervalSince1970: 1553105830 / 1000))
        })
    }
    
    public func testGetNep11Transfers_DateFromTo() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep11transfers\"," +
        "\"id\":1," +
        "\"params\":[\"NSH1UeM96PKhjuzVBKcyWeNNuQkT3sHGmA\",1553105830,1557305830]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep11Transfers(Hash160("8bed27d0e88266807a6339270f0593510967cb45"),
                                                      Date(timeIntervalSince1970: 1553105830 / 1000),
                                                      Date(timeIntervalSince1970: 1557305830 / 1000))
        })
    }
    
    public func testGetNep11Properties() {
        let json = "{\"jsonrpc\":\"2.0\"," +
        "\"method\":\"getnep11properties\"," +
        "\"id\":1," +
        "\"params\":[\"NfWL3Kx7qtZzXrajmggAD4b6r2kGzajbaJ\",\"12345\"]}"
        
        verifyRequest(json, { rpcClient in
            return try! rpcClient.getNep11Properties(Hash160("2eeda865e7824c71b3fe14bed35d04d0f2f0e9d6"),"12345")
        })
    }
    
    private func verifyRequest<T: Response<U>, U>(_ expected: String, config: NeoRpcClientConfiguration = .init(), _ makeRequest: (NeoRpcClient) throws -> Request<T, U>) {
        let httpService = HttpService(urlSession: MockURLSession())
        let rpcClient = NeoRpcClient.build(httpService, config)
        NeoRpcClientConfiguration.REQUEST_COUNTER.reset()
        let request: Request<T, U> = try! makeRequest(rpcClient)
        assertJsonRequestBody(try! requestPayload(request), equals: expected)
    }
    
    private func verifyExpressRequest<T: Response<U>, U>(_ expected: String, _ makeRequest: (NeoExpressClient) throws -> Request<T, U>) {
        let httpService = HttpService(urlSession: MockURLSession())
        let rpcClient: NeoExpressClient = NeoExpressClient.build(httpService)
        NeoRpcClientConfiguration.REQUEST_COUNTER.reset()
        let request: Request<T, U> = try! makeRequest(rpcClient)
        assertJsonRequestBody(try! requestPayload(request), equals: expected)
    }
    
    private func requestPayload<T: Response<U>, U>(_ request: Request<T, U>) throws -> Data {
        let encoder = JSONEncoder()
        encoder.outputFormatting = .withoutEscapingSlashes
        return try encoder.encode(request)
    }

    private func assertJsonRequestBody(_ body: Data, equals expected: String) {
        guard let expectedData = expected.data(using: .utf8) else {
            XCTFail("Could not encode expected JSON request body.")
            return
        }

        do {
            let actualJson = try JSONSerialization.jsonObject(with: body)
            let expectedJson = try JSONSerialization.jsonObject(with: expectedData)
            let actualNormalized = try normalizedJsonString(actualJson)
            let expectedNormalized = try normalizedJsonString(expectedJson)
            XCTAssertEqual(actualNormalized, expectedNormalized)
        } catch {
            let actualBody = String(data: body, encoding: .utf8) ?? "<non-utf8 body>"
            XCTFail("""
                    Failed to compare JSON request bodies: \(error)
                    actual: \(actualBody)
                    expected: \(expected)
                    """)
        }
    }

    private func normalizedJsonString(_ jsonObject: Any) throws -> String {
        let data = try JSONSerialization.data(withJSONObject: jsonObject, options: [.sortedKeys])
        guard let jsonString = String(data: data, encoding: .utf8) else {
            throw NeoError.illegalState("Could not decode normalized JSON to UTF-8 string.")
        }
        return jsonString
    }
    
}
