        ...
        let json = """
{
    ...
    "result": {
        ...
        "mode": "Sign(payload)",
        "payload": "010001f00568656c6c6f0000",
        "signatures": [
            {
...
"""
        let result = decodeJson(NeoSignMessage.self, from: json).signMessage!
        XCTAssertEqual(result.curve, "secp256r1")
        XCTAssertEqual(result.signatures.count, 1)
        XCTAssertEqual(result.signatures[0].publicKey, "02ec143f00b88524caf36a0121c2de09eef0519ddbe1c710a00f0e2663201ee4c0")
    }
...
"""
        let result = decodeJson(NeoVerifyMessage.self, from: json).verifyMessage!
        XCTAssertTrue(result.isValid)
        XCTAssertEqual(result.status, "Valid")
    }
    
    public func testPendingValidUntilRelay() {
        let json = """
{
    "jsonrpc": "2.0",
    "id": 1,
    "result": {
        ...
        "enabled": true,
        "pendingcheckfrequency": 10,
        "pendingrelaymaxtransactions": 1000,
        "pending": [
            {
                "hash": "0x9f0c9be9507d545b4ed3311d3c5efed7c3df91d3502149ac828f036946b34022",
                "validuntilblock": 120,
                "size": 128,
                "blocksuntildeadline": 20
            }
        ],
        "count": 1
    }
...
"""
        let result = decodeJson(NeoGetPendingValidUntilRelay.self, from: json).pendingState!
        XCTAssertTrue(result.enabled)
        XCTAssertEqual(result.pending.count, 1)
        XCTAssertEqual(result.pending[0].blocksUntilDeadline, 20)
    }
...
"""
    ...
    public func testGetVersionNeo310ProtocolMetadata() {
        let json = """
{
    "jsonrpc": "2.0",
    "id": 1,
...
"""
        ...
        XCTAssertEqual(version.userAgent, "/Neo:3.10.0/")
        XCTAssertEqual(version.rpc?.maxIteratorResultItems, 100)
        XCTAssertEqual(version.rpc?.sessionEnabled, true)

        let p = version.protocol!
        XCTAssertEqual(p.network, 860833102)
        XCTAssertEqual(p.addressVersion, 53)
        XCTAssertEqual(p.validatorsCount, 7)
        XCTAssertEqual(p.maxValidUntilBlockIncrement, 5760)
        XCTAssertEqual(p.initialGasDistribution, 5_200_000_000_000_000)
        XCTAssertEqual(p.hardforks, [.init(name: "Aspidochelone", blockHeight: 1_730_000)])
        XCTAssertEqual(p.standbyCommittee, ["02a7834be9b32e2981d157cb5bbd3acb42cfd11ea5c3b10224d7a44e98c5910f1b"])
        XCTAssertEqual(p.seedList, ["seed1.neo.org:10333"])
    }
...
"""
    ...
    public func testInvokeFunctionNeo310DiagnosticsTraces() {
        let json = """
{
    "jsonrpc": "2.0",
    "id": 1,
...
"""
        let diagnostics = decodeJson(NeoInvokeFunction.self, from: json).invocationResult!.diagnostics!
        XCTAssertTrue(diagnostics.storageChanges.isEmpty)
        XCTAssertEqual(diagnostics.traces?.type, "root")
        XCTAssertEqual(diagnostics.traces?.calls?.count, 1)
        XCTAssertEqual(diagnostics.traces?.calls?.first?.method, "symbol")
        XCTAssertEqual(diagnostics.traces?.calls?.first?.isNative, true)
        XCTAssertEqual(try diagnostics.traces?.calls?.first?.returnValue?.getString(), "NEO")
    }
...
"""
    ...
    func decodeJson<T: Decodable>(_ type: T.Type, from json: String) -> T {
