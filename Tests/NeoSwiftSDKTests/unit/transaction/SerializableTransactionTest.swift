...
class SerializableTransactionTest: XCTestCase {
    ...
    let rpcClient = NeoRpcClient.build(HttpService(url: URL(string: "http://localhost:40332")!))

    public func testSerializeWithoutAttributesAndWitnesses() {
        let signers = try! [AccountSigner.calledByEntry(account1)]
        let tx = NeoTransaction(rpcClient: rpcClient, version: 0, nonce: 0x01020304,
    ...
    public func testDeserialize() {
        ...
        XCTAssertNil(transaction.rpcClient)
        transaction.rpcClient = .build(HttpService(url: URL(string: "http://localhost:40332")!))
        XCTAssertNotNil(transaction.rpcClient)
    }
    ...
    public func testGetTxId() {
        ...
        let tx = NeoTransaction(rpcClient: rpcClient, version: 0, nonce: 226292130,
    ...
    public func testGetHashData() async {
        ...
        let expected = try! await rpcClient.getNetworkMagicNumberBytes() + withoutWitness.sha256()
    ...
    public func testContractParameterContextJson() async {
        ...
        let rpcClient = NeoRpcClient(config: .init(networkMagic: 769), service: service)
