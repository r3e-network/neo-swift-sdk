...
class SmartContractTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let rpcClient = NeoRpcClient.build(HttpService(urlSession: mockUrlSession))
        someContract = SmartContract(scriptHash: SOME_SCRIPT_HASH, rpcClient: rpcClient)
        neoContract = SmartContract(scriptHash: NEO_SCRIPT_HASH, rpcClient: rpcClient)
    }
    ...
    public func testInvokeWithEmptytring() {
        XCTAssertThrowsError(try neoContract.invokeFunction("", [])) { error in
            guard case NeoError.illegalArgument(let message) = error else {
                return XCTFail()
            }
    ...
    public func testCallFunctionTraversingIteratorTerminatesSessionWhenMappingThrows() async throws {
        ...
        _ = mockUrlSession
            .requestInterceptor { request in
                ...
                terminatedSession = object["method"] as? String == "terminatesession"
            }
            .data(["traverseiterator": JSON.from("traverseiterator"),
                   "terminatesession": JSON.from("terminatesession")])
            .invokeFunctions(["tokens": JSON.from("invokefunction_iterator_session")])
        ...
        do {
            _ = try await someContract.callFunctionAndTraverseIterator("tokens") { _ in
                throw NeoError.illegalState("mapping failed")
            }
            XCTFail("Expected mapper failure")
        } catch NeoError.illegalState(let message) {
            XCTAssertEqual(message, "mapping failed")
        } catch {
            XCTFail("Unexpected error: \(error)")
        }

        XCTAssertTrue(terminatedSession)
    }
    ...
    public func testCallInvokeFunction_missingFunction() async throws {
        ...
        do {
            ...
            XCTFail("No exception")
        } catch NeoError.illegalArgument(let message) {
            XCTAssertEqual(message, "The invocation function must not be empty.")
        } catch {
            XCTFail("Incorrect exception.")
        }
